...
def is_unacceptable(score, total_q=180):
    """Checks if a NEET score is mathematically possible (+4/-1 system)"""
    max_score = total_q * 4
    min_score = -total_q
    ...
    # Mathematical validation: S = 4C - I where C+I <= 180
...
if st.button("Predict Rank"):
    if not name:
        ...
        # Check if the score is mathematically possible
        if is_unacceptable(score) == "<unacceptable marks>":
            st.error(f"⚠️ {score} is an impossible score. Please check your calculation (Correct: +4, Incorrect: -1).")
        else:
            ...
            if score >= 716:
                ...
                result = f"{lower:,} - {upper:,}" # Added comma formatting for readability
